...
function AddDoor() {
  ...
  const handleSubmit = async (e) => {
    ...
    fetch(
      "http://localhost:8080/api/locks/register?tokenUser=" +
      ...
      if (res.status === 400) {
        MySwal.fire({
          icon: "error",
          title: "ooops",
          text: "No se ha podido agregar la puerta",
        });
      } else {
        MySwal.fire({
          icon: "success",
  ...
  return (
    <div>
        ...
        <Container>
          ...
          <Row>
            <Col>
              <Button
                variant="dark"
                onClick={handleSubmit} //llamo al metodo para su funcionalidad
              >
                Agregar
              </Button>
            </Col>
      ...
      <Button variant="dark" href="/myDoors">
        Volver
      </Button>
    </div>
  );
